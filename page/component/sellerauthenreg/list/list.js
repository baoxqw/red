...
Page({
  ...
  formSubmit: function(e) {
    ...
    if (!e.detail.value.shopname.length || !e.detail.value.address.length || !e.detail.value.sellername.length || !that.data.nofiles.length) {
      wx.showToast({
        title: '将表单填写完整！',
        icon: 'none',
        duration: 1500
    ...
    wx.request({
      url: u+"/jujube/register/seller/insert",
      header: header, //请求时带上这个请求头
      method: 'post',
      data: ({
