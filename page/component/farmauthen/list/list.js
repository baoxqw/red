...
Page({
  ...
  onLoad: function (options) {
    ...
    // //显示列表信息
    wx.request({
      url: u+"/jujube/register/company/get",
      ...
      success: function (res) {
        ...
        if (res.data.data) {
          ...
          if (lists.auditstate == 1) {
            ...
            that.setData({ state: "审核成功" })
          }
          else {
            that.setData({ state: "审核失败" })
          }
          that.setData({
            id: lists.id,
            lists: lists,
  ...
  birclearImg: function (e) {
    let that = this;
    var index = e.currentTarget.dataset.index;
    let evalList = this.data.birthfiles;
    let noevalList = this.data.nobirthfiles;
    evalList.splice(index, 1);
    noevalList.splice(index, 1);
    that.setData({
      birthfiles: evalList,
          ...
          wx.uploadFile({
            ...
            success: function (res) {
              ...
              if (resp.msg == "上传成功") {
                ...
                if (resp.code == 0) {
                  that.setData({
                    birthfiles: that.data.files.concat(imgpath),
  ...
  formSubmit: function (e) {
    ...
    if (!e.detail.value.shopname.length || !e.detail.value.sellername.length || !e.detail.value.idcardnum.length
      || !e.detail.value.creditcode.length || !e.detail.value.address.length || !e.detail.value.telphone.length
    ) {
      wx.showToast({
        title: '将表单填写完整！',
        icon: 'none',
    ...
    if (that.data.nobirthfiles.length < 2) {
      wx.showToast({
        title: "请上传身份证正反面照片",
        icon: 'none',
        duration: 2000
    ...
    wx.request({
      url: u+"/jujube/register/company/update",
      header: header, //请求时带上这个请求头
      method: 'post',
      data: ({
        "id": that.data.id,
