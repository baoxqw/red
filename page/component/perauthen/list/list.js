...
Page({
  ...
  onLoad: function(options) {
    ...
    // //显示列表信息
    wx.request({
      url: u+"/jujube/register/self/get",
      ...
      success: function(res) {
        ...
        //身份证图片
        if (lists.idcardpic) {
          let birthpic = lists.idcardpic.split(',')
          let newbirthpic = []
  ...
  formSubmit: function(e) {
    ...
    if (!e.detail.value.shopname.length || !e.detail.value.address.length || !e.detail.value.tuaddress.length || !e.detail.value.telphone.length) {
      wx.showToast({
        title: '将表单填写完整！',
        icon: 'none',
        duration: 1500
    ...
    wx.request({
      url: u+"/jujube/register/self/update",
      header: header, //请求时带上这个请求头
      method: 'post',
      data: ({
        "id": that.data.id,
        "name": e.detail.value.shopname, //姓名
        ...
        "landlocation": e.detail.value.tuaddress, //土地所在地
      }),
      success: function(res) {
        wx.hideLoading();
        that.setData({
