...
Page({
  ...
  onLoad: function(options) {
    ...
    // //显示列表信息
    wx.request({
      url: u+"/jujube/register/seller/get",
      ...
      success: function(res) {
        if (res.data.data) {
          ...
          if (lists.auditstate == 1) {
            that.setData({
              state: "审核中",
              sure: true,
              num: 2,
            })
          } else if (lists.auditstate == 2) {
            that.setData({
              state: "审核成功"
            })
          } else {
        ...
        //单选框选中
        let radioValue = lists.selltype;
        if (lists.selltype == 1) {
          that.data.radioItems[0].checked = true;
        } else if (lists.selltype == 2) {
          that.data.radioItems[1].checked = true;
        } else if (lists.selltype == 3) {
          that.data.radioItems[2].checked = true;
        }
        //信用代码图片
        if (lists.creditcodepic) {
          let pic = lists.creditcodepic.split(',');
          let newpic = []
          for (let i in pic) {
            newpic.push(getApp().globalData.imgPath + pic[i])
          }
          that.setData({
            files: newpic,
            nofiles: pic
        ...
        //身份证图片
        if (lists.idcardnumpic) {
          let birthpic = lists.idcardnumpic.split(',')
          let newbirthpic = []
          for (let i in birthpic) {
            newbirthpic.push(getApp().globalData.imgPath + birthpic[i])
          }
          that.setData({
            birthfiles: newbirthpic,
            nobirthfiles: birthpic
          })
        }
      }
    })
  ...
  formSubmit: function(e) {
    ...
    if (!e.detail.value.telphone.length || !e.detail.value.shopname.length || !e.detail.value.address.length || !e.detail.value.sellername.length || !that.data.files.length) {
      wx.showToast({
        title: '将表单填写完整！',
        icon: 'none',
        duration: 1500
