...
Page({
  ...
  onLoad: function (options) {
    ...
    // //显示列表信息
    wx.request({
      url: u+"/jujube/growthrecord/list",
      header: header, //请求时带上这个请求头
      method: 'post',
      data: ({
        pageNo: 1,
      ...
      success: function (res) {
        ...
        if(res.data.code == 200){
          if (res.data.data.list) {
            lists.forEach(v => {
              // v.growthrecordtime = v.growthrecordtime.substring(0, 10);
              v.growthrecordtime = moment(v.growthrecordtime).format('l');
            })
  ...
  yes: function () {
    ...
    wx.request({
      url: u+"/jujube/growthrecord/delete",
      header: header, //请求时带上这个请求头
      method: 'get',
      data: ({
