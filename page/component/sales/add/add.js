...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        that.setData({ typearray: salesArray })
      }
    })
  },
  //点击产品名称
  ...
  */
  ...
  bindShowSalesMsg() {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        that.setData({ selectSales: newsalesArray })
      }
    })
  },
  //文本域输入内容
  ...
  formSubmit: function (e) {
    ...
    if (!e.detail.value.num.length || !e.detail.value.address.length 
       || !that.data.productId || !that.data.salesId) {
      wx.showToast({
        title: '将表单填写完整',
        icon: 'none',
        duration:2000
