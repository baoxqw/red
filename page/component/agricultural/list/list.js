...
Page({
  ...
  onLoad: function(options) {
    ...
    // //显示列表信息
    wx.request({
      url: u+"/jujube/register/purchase/keyFindPurchase",
      header: header, //请求时带上这个请求头
      method: 'get',
      data: ({
        currentPage: 1,
      ...
      success: function(res) {
        ...
        lists.forEach(v => {
          v.purchasetime = moment(v.purchasetime).format('l');
        })
        that.setData({
          lists: lists
        })
  ...
  yes: function() {
    ...
    wx.request({
      url: u+"/jujube/register/catalog/delete",
      header: header, //请求时带上这个请求头
      method: 'post',
      data: ({
    ...
    wx.request({
      ...
      success: function(res) {
        ...
        if (lists.length == 0) {
          ...
          lists.forEach(v => {
            // v.picktime = v.picktime.substring(0, 10);
            v.purchasetime = moment(v.purchasetime).format('l');
