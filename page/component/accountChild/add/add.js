...
Page({
  ...
  formSubmit: function(e) {
    ...
    // }
    if (!e.detail.value.name.length || !that.data.manageid || !that.data.confirmValue || !that.data.passwordValue) {
      wx.showToast({
        title: '将表单填写完整',
        icon: 'none',
        duration: 2000
    ...
    wx.request({
      url: u+"/jujube/system/user/add",
      header: header, //请求时带上这个请求头
      method: 'post',
      data: ({
        "username": that.data.userValue,
        "password": that.data.passwordValue,
        "telphone": that.data.phoneValue,
